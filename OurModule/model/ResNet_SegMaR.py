...
class Generator(nn.Module):
    ...
    def forward(self, x):
        fix_pred, cod_pred2 = self.sal_encoder(x)
...
class Fix_feat_decoder2(nn.Module):
    # resnet based encoder decoder
    def __init__(self, channel):
        super(Fix_feat_decoder2, self).__init__()
        self.relu = nn.ReLU(inplace=True)
        self.upsample8 = nn.Upsample(scale_factor=8, mode='bilinear', align_corners=True)
        self.upsample4 = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True)
        self.upsample2 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        ...
        self.racb4 = RCAB(channel * 3)

        self.cls_layer = self._make_pred_layer(Classifier_Module, [6, 12, 18, 24], [6, 12, 18, 24], 1, channel * 3)
    ...
    def forward(self, x2,x3,x4):
        ...
        conv4321 = torch.cat((self.upsample2(conv2_feat),self.upsample4(conv3_feat), self.upsample8(conv4_feat)),1)
...
class Saliency_feat_encoder(nn.Module):
    ...
    def __init__(self, channel):
        ...
        self.sal_dec = Fix_feat_decoder(channel)
        self.HA = HA()
    ...
    def forward(self, x):
        ...
        x2_2 = self.HA(self.upsample05(fix_pred).sigmoid(), x2)
        ...
        return self.upsample4(fix_pred),self.upsample4(ref_pred)
